...
// 	char intClient[16];
// 	char intPort[5];
// 	char desc[20];//upnp注册名
// 	char rHost[20];
// 	char duration[20];
// }UpnpPacket;

char strExternalIP[16] = {0};
                         ...
                         struct IGDdatas * data)
{
	...
	if(r != UPNPCOMMAND_SUCCESS) {
		...
		strcpy(strExternalIP, externalIPAddress);
...
static void ListRedirections(struct UPNPUrls * urls,
                             struct IGDdatas * data,
							 multimap<string, UpnpPacket> &mmap)
{
	int r;
	int i = 0;
	char index[6];
	...
	do {
		...
		r = UPNP_GetGenericPortMappingEntry(urls->controlURL,
									   ...
									   rHost, duration);
		if(r==0){
			UpnpPacket _upnpPacket;
			strcpy(_upnpPacket.protocol, protocol);
			strcpy(_upnpPacket.extPort, extPort);
			strcpy(_upnpPacket.intPort, intPort);
			strcpy(_upnpPacket.intClient, intClient);
			strcpy(_upnpPacket.desc, desc);
			strcpy(_upnpPacket.rHost, rHost);
			strcpy(_upnpPacket.duration, duration);
			mmap.insert(make_pair(intClient, _upnpPacket));
		}
		/*
...
int upnpSetup(char *pExIpaddress,int argc, char ** argv, multimap<string, UpnpPacket> &mmap)
{
	                             ...
	                             0/*sameport*/, ipv6, &error)))
	{
		  ...
		  || (i = UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr))))
		{
			...
			case 'l':
				DisplayInfos(&urls, &data);
				strcpy(pExIpaddress, strExternalIP);
				ListRedirections(&urls, &data, mmap);
				break;
			case 'L':
			...
			case 'a':
				SetRedirectAndTest(&urls, &data,
				                   ...
				                   (commandargc > 4)?commandargv[4]:"0",externalIPAddress);
				printf("ExternalIPAddress = %s\n", externalIPAddress);
				strcpy(pExIpaddress,externalIPAddress);
				break;
			case 'd':
			...
			case 'r':
				for(i=0; i<commandargc; i+=2)
				{
					...
					SetRedirectAndTest(&urls, &data,
									   ...
									   commandargv[i], commandargv[i+1], "0",externalIPAddress);
				}
				break;
			case 'A':
...
int countfreeport(struct UPNPUrls * urls,
                             struct IGDdatas * data)
{
	int freeport=0;
	int r;
	...
	int extPortArray[200]={0};
	int inPortArray[200]={0};
	...
	do {
		...
		if(r==0)
		{
			...
			/*插入数组*/
			extPortArray[i]=atoi(extPort);
			inPortArray[i]=atoi(intPort);
		}
	...
	exNode=insertCreateSetLink(extPortArray);
	inNode=insertCreateSetLink(inPortArray);
...
int getfreeport()
{
	                             ...
	                             0/*sameport*/, ipv6, &error)))
	{
		  ...
		  || (i = UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr))))
		{
			...
			/*获取可用端口*/
			freeport=countfreeport(&urls, &data);
...
{
	int freeport = atoi(pInternalport);

	//freeport=getfreeport();

//	char *argv[] = {"-a", "192.168.1.101","8086", "8086","TCP"};
	char *argv[5];
	char argvs[5][255];

	strcpy(argvs[0],"-a");
	strcpy(argvs[1], pIpaddress);
	strcpy(argvs[2], pInternalport);
	sprintf(argvs[3],"%d",freeport);
	strcpy(argvs[4],"TCP");
	//sprintf(pExternalPort,"%d",freeport);
	...
	upnpSetup(pExIpaddress,5, argv, mmap);
